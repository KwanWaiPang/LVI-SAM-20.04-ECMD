...
//将img_callback中的const sensor_msgs::ImageConstPtr 改为const sensor_msgs::CompressedImageConstPtr
void img_callback(const sensor_msgs::CompressedImageConstPtr &Compressedimg_msg)
{
    //解压图像数据
    cv::Mat image=cv::imdecode(cv::Mat(Compressedimg_msg->data),cv::IMREAD_COLOR);

    //创建为sensor_msgs::ImageConstPtr &img_msg
    // const sensor_msgs::ImageConstPtr img_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", image).toImageMsg();
    const sensor_msgs::ImageConstPtr img_msg = cv_bridge::CvImage(Compressedimg_msg->header, "bgr8", image).toImageMsg();
