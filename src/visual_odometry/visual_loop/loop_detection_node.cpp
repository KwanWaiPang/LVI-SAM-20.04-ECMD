...
void image_callback(const sensor_msgs::CompressedImageConstPtr &Compressedimg_msg)
{
    //解压图像数据
    cv::Mat image=cv::imdecode(cv::Mat(Compressedimg_msg->data),cv::IMREAD_COLOR);

    //创建为sensor_msgs::ImageConstPtr &image_msg
    const sensor_msgs::ImageConstPtr image_msg = cv_bridge::CvImage(Compressedimg_msg->header, "bgr8", image).toImageMsg();
